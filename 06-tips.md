---
layout: default
title: 06.加工(2)
nav_order: 7
---

# 06.加工(2)
<br><br>

## ★一時停止、加工中断する
<br>

**（方法1）操作パネルでの一時停止操作**

<img src="assets/06-1.jpg" width="320" alt="hi" class="inline"/><br>

マシン操作パネルの一時停止ボタンを押すと加工が一時停止する。<br>
再開する場合は同じボタンをもう一度押す。<br>
<br><br>

**（方法2）JobControlでの一時停止、加工中断操作**

<img src="assets/06-2.png" width="320" alt="hi" class="inline"/><br>

画面右下の一時停止ボタンを押すと加工が一時停止し、白色四角のボタンを押すと加工が終了する。<br>
一時停止ボタンは、もう一度押すと加工が再開される。<br>
<br>
<br>
<br>

## ★同じ加工データでもう一度加工する
<br>

<img src="assets/06-3.png" width="640" alt="hi" class="inline"/><br>

加工データは、一度使用するとそのままでは再度使用できない。<br>
再度加工したい場合は、**データの上で右クリック** をし、**“ジョブを再起動”**を選択する。<br>
<br>
<br>
<br>

## ★加工データの向きを変える
<br>

<img src="assets/06-4.png" width="320" alt="hi" class="inline"/> <img src="assets/06-5.png" width="320" alt="hi" class="inline"/><br>

向きを変えたいデータの上で右クリックをし、**“ジョブの方向を変える”**をクリックする。<br>
<br>
<br>
<br>

## ★原点の位置を数値で設定する
<br>

<img src="assets/06-6.png" width="640" alt="hi" class="inline"/><br>

はじめに、マシンとPCを接続し、マシンの蓋を閉める。<br>
その後、 **“レーザー彫刻機”** ＞ **“レーザーを指定位置に移動”**を選択する。<br>
<br>

<img src="assets/06-7.png" width="640" alt="hi" class="inline"/><br>

任意の値を入力し**「OK」**を押すと原点の位置が移動する。<br>
